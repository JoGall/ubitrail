...
#' area. Each row in the matrices represents a read (\emph{i.e.} an 
...
#' This is performed individually for each area. Although this is usually fast and negligible, 
#' in some conditions (\emph{e.g.} if the agent is static or hides) it can take some time.
...
#' In most cases, this can be interpreted as an immobile behaviour.
